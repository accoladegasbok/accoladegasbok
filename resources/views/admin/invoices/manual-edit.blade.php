{{-- FILE: resources/views/admin/invoices/manual-edit.blade.php --}}
@extends('admin.layouts.admin')
@section('title', 'Edit Invoice')
@section('page-title', 'Edit Invoice — ' . ($invoice->invoice_no ?? ('#'.$invoice->id)))
@section('page-sub', 'Update customer, items, or pricing on this invoice')

@section('content')

@if($errors->any())
<div class="bg-red-50 border border-red-300 rounded-2xl px-5 py-4 mb-5 max-w-5xl">
...
<form method="POST" action="{{ route('admin.invoices.manual.update', $invoice->id) }}" id="invoiceForm">
@csrf
@method('PUT')

{{-- Supervisor override token — hidden until PIN modal approves --}}
<input type="hidden" name="override_token" id="overrideTokenInput" value="">
...
<div class="max-w-5xl space-y-5">
  ...
  <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
      <div class="col-span-2 sm:col-span-1 relative">
        ...
        <input type="text" name="customer_phone" id="customerPhoneInput" value="{{ old('customer_phone', $invoice->customer_phone) }}" placeholder="+1 512 000 0000" autocomplete="off"
      ...
      <div class="col-span-2 sm:col-span-1">
        ...
        <input type="text" name="customer_name" id="customerNameInput" required value="{{ old('customer_name', $invoice->customer_name) }}" placeholder="Full name or company"
          class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-gold">
      </div>
      <div>
        <label class="block text-xs text-gray-500 uppercase tracking-wider mb-1">Email (optional)</label>
        <input type="email" name="customer_email" id="customerEmailInput" value="{{ old('customer_email', $invoice->customer_email) }}" placeholder="customer@email.com"
          class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-gold">
      </div>
      <div class="col-span-2 sm:col-span-3">
        <label class="block text-xs text-gray-500 uppercase tracking-wider mb-1">Address (optional)</label>
        <input type="text" name="customer_address" id="customerAddressInput" value="{{ old('customer_address', $invoice->customer_address) }}" placeholder="Street, city, etc."
          class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-gold">
      </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-3">
      <div>
        ...
        <select name="location" id="locationSelect" onchange="updateCurrency()" required
          ...
          <option value="{{ $key }}" {{ old('location', $invoice->location) === $key ? 'selected' : '' }}>{{ $label }}</option>
          @endforeach
        </select>
      </div>
      <div>
        ...
        <select name="payment_method"
          class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:border-gold">
          @foreach(['Cash','Bank Transfer','Moniepoint','POS','Zelle','CashApp','Venmo','Paystack','Credit / Deferred'] as $pm)
          <option {{ old('payment_method', $invoice->payment_method) === $pm ? 'selected' : '' }}>{{ $pm }}</option>
          @endforeach
        </select>
      </div>
      <div class="col-span-2">
        <label class="block text-xs text-gray-500 uppercase tracking-wider mb-1">Notes (optional)</label>
        <input type="text" name="notes" value="{{ old('notes', $invoice->notes) }}" placeholder="Any special notes for this invoice..."
          class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-gold">
      </div>
    </div>
  </div>
  ...
  <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
    <div class="flex items-center justify-between px-5 py-3 bg-navy">
      ...
      <div class="flex items-center gap-2">
        <span id="currencyDisplay" class="text-gold font-mono font-700 text-sm">{{ $currency['code'] ?? 'USD' }} ({{ $currency['symbol'] ?? '$' }})</span>
    ...
    <div id="itemsContainer" class="divide-y divide-gray-100 p-4 space-y-3">
      {{-- Items seeded from existing invoice via JS on load --}}
    </div>
        ...
        <div class="flex justify-between items-center text-sm font-body py-1">
          ...
          <div class="flex gap-1 items-center">
            <input type="number" name="invoice_discount_value" id="invoiceDiscountInput"
                value="{{ old('invoice_discount_value', $invoice->discount_value ?? 0) }}" min="0" oninput="updateTotal()"
            ...
            <select name="invoice_discount_type" id="invoiceDiscountType" onchange="updateTotal()"
                ...
                <option value="fixed" {{ old('invoice_discount_type', $invoice->discount_type ?? 'fixed') === 'fixed' ? 'selected' : '' }}>$</option>
                <option value="percent" {{ old('invoice_discount_type', $invoice->discount_type ?? 'fixed') === 'percent' ? 'selected' : '' }}>%</option>
            </select>
          </div>
        </div>
      ...
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1">
          ...
          <input type="text" name="buyer_legal_doc" id="buyerLegalDocInput"
                 value="{{ old('buyer_legal_doc', $invoice->buyer_doc_ref ?? '') }}"
  ...
  {{-- Supervisor Approval Notice --}}
  @if(session('staff_role') === 'supervisor')
  <div class="bg-amber-50 border border-amber-300 rounded-2xl p-4 flex items-start gap-3">
    <span class="text-xl">🔐</span>
    <div class="flex-1">
      <div class="font-700 text-amber-800 text-sm">Supervisor approval required</div>
      <p class="text-xs text-amber-700 mt-0.5">Saving this edit will prompt for a supervisor PIN. Every edit is logged for audit.</p>
    </div>
  </div>
  @endif
  ...
  <div class="flex gap-3 justify-end pb-8">
    ...
    <button type="{{ session('staff_role') === 'supervisor' ? 'button' : 'submit' }}"
      @if(session('staff_role') === 'supervisor') onclick="requestSupervisorApproval()" @endif
...
function addItem(seed = null) {
    ...
    // Pre-fill from existing invoice item if seeding
    if (seed) {
        document.getElementById('item-name-' + i).value           = seed.name || '';
        document.getElementById('item-pid-' + i).value             = seed.part_id || '';
        document.getElementById('item-price-' + i).value           = seed.price || 0;
        document.getElementById('item-grade-' + i).value           = seed.grade || 'B';
        document.getElementById('item-qty-' + i).value             = seed.qty || 1;
        document.getElementById('item-discount-' + i).value        = seed.discount_value || 0;
        document.getElementById('item-discount-type-' + i).value   = seed.discount_type || 'fixed';
    }
...
function requestSupervisorApproval() {
    ...
    requestOverride('edit_invoice', 'Edit Invoice #{{ $invoice->id }}', function(approvedBy, role) {
        document.getElementById('overrideTokenInput').value = `${approvedBy} (${role})`;
        document.getElementById('invoiceForm').submit();
    });
}

// ── Initialize: seed currency + existing items ──────────────────
updateCurrency();
