...
@section('page-title', 'Edit Invoice ' . $invoice->invoice_no)
@section('page-sub', 'Editing — changes are logged. Originally created by ' . $invoice->created_by)
@section('content')
...
<div class="max-w-5xl space-y-5">
  ...
  <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
      <div class="col-span-2 sm:col-span-1">
        ...
        <input type="text" name="customer_name" value="{{ $invoice->customer_name }}" required placeholder="Full name or company"
      ...
      <div>
        ...
        <input type="text" name="customer_phone" value="{{ $invoice->customer_phone }}" placeholder="+1 512 000 0000"
          class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-gold">
      </div>
      <div>
        <label class="block text-xs text-gray-500 uppercase tracking-wider mb-1">Email (optional)</label>
        <input type="email" name="customer_email" value="{{ $invoice->customer_email }}" placeholder="customer@email.com"
          class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-gold">
      </div>
      <div>
        <label class="block text-xs text-gray-500 uppercase tracking-wider mb-1">Email (optional)</label>
  ...
  <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
    ...
    <option value="{{ $key }}" {{ $invoice->location === $key ? 'selected' : '' }}>{{ $label }}</option>
      @endforeach
        </select>
      </div>
      <div>
...
// Pre-fill existing invoice items instead of adding one blank row
...
if (existingItems.length > 0) {
    existingItems.forEach(function(item) {
        addItem();
        const i = itemCount;
        document.getElementById('item-name-' + i).value = item.name;
        if (item.part_id) document.getElementById('item-pid-' + i).value = item.part_id;
        document.getElementById('item-price-' + i).value = currency.code === 'NGN'
            ? Math.round(item.price) : item.price.toFixed(2);
        document.getElementById('item-grade-' + i).value = item.grade || 'B';
        document.getElementById('item-qty-' + i).value = item.qty;
        if (item.discount_value) {
            document.getElementById('item-discount-' + i).value = item.discount_value;
            document.getElementById('item-discount-type-' + i).value = item.discount_type;
        }
    });
} else {
    addItem();
...
// Pre-fill invoice-level discount
@if($invoice->discount_value)
document.getElementById('invoiceDiscountInput').value = {{ $invoice->discount_value }};
document.getElementById('invoiceDiscountType').value = '{{ $invoice->discount_type }}';
@endif
