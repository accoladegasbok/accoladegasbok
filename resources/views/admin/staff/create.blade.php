...
@section('page-sub','Create a new staff login and assign their role')
...
<div class="max-w-2xl">
  <form method="POST" action="{{ route('admin.staff.store') }}">
    ...
    {{-- Commission — only shown/relevant for sales_rep, admin-settable only --}}
    @if(session('staff_role') === 'admin')
    <div class="stat-card mb-5" id="commissionCard" style="display:none;">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-3">
        <div>
          ...
          <input type="number" name="commission_base_percent" value="{{ old('commission_base_percent') }}" step="0.1" min="0" max="100" placeholder="e.g. 2.0"
            class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm font-body focus:outline-none focus:border-yellow-400">
        </div>
      </div>
      <div>
...
function addTierRow() {
    ...
    row.innerHTML = `
        <input type="number" placeholder="Min volume (e.g. 500000)" step="0.01" min="0"
        ...
        <input type="number" placeholder="Percent (e.g. 3)" step="0.1" min="0" max="100"
