...
@section('page-sub','Auto-built from order and invoice history — search by name or phone')
@section('content')
...
<div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 mb-5">
  <form method="GET" action="{{ route('admin.customers.index') }}" class="flex gap-3">
    <input type="text" name="q" value="{{ $search }}" placeholder="Search by name or phone number..."
  ...
  <table class="w-full text-sm">
    ...
    <tbody>
      ...
      <tr class="border-b border-gray-50 hover:bg-gray-50 transition-colors">
        ...
        <td class="px-4 py-3 text-gray-500 text-xs">{{ \Carbon\Carbon::parse($c->last_purchase)->format('M j, Y') }}</td>
        <td class="px-4 py-3 text-right">
          <a href="{{ route('admin.customers.show', $c->phone) }}"
            class="text-gold hover:text-yellow-600 font-body font-500 text-xs">
            View History →
          </a>
        </td>
      </tr>
      ...
      <tr>
        <td colspan="7" class="px-4 py-8 text-center text-gray-400 text-sm">
          ...
          @else
            No customer records yet — they'll appear here automatically as orders and invoices come in.
          @endif
        </td>
      </tr>
      @endforelse
